...
public class PlayerControllerIKFeetPlacementTorqueTerrainVegFinalTest : MonoBehaviour
{
    #region Variables

    public FootGroundDetector rightFootDetector;
    //public FootGroundDetector rightToeDetector;
    public FootGroundDetector leftFootDetector;
    //public FootGroundDetector leftToeDetector;
    ...
    private PlayerControllerIKFeetPlacementTorqueTerrainVegFinalTest redScript;
    ...
    void Start()
    {
        ...
        Debug.Log("Pieds : " + leftFootIKPosition + " " + rightFootIKPosition);

        // OBTAINING OTHER IK GOAL
        if (addOFFSETFOOT)
            redScript = redPlayer.GetComponent<PlayerControllerIKFeetPlacementTorqueTerrainVegFinalTest>();
    ...
    private void FixedUpdate()
    {
        ...
        leftFootIKPosition = shoesLeft.transform.position;
        rightFootIKPosition = shoesRight.transform.position;
    ...
    void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireSphere(
            groundCheckerLeftFoot.position,
            groundDistanceFoot
        );
    }
    ...
    private void CheckFeetAreGrounded()
    {
        ...
        {
            if (Physics.CheckSphere(groundCheckerLeftFoot.position, groundDistanceFoot, ground) || Physics.CheckSphere(groundCheckerLeftFootBack.position, groundDistanceFoot, ground))
            {
        ...
        {
            if (Physics.CheckSphere(groundCheckerRightFoot.position, groundDistanceFoot, ground) || Physics.CheckSphere(groundCheckerRightFootBack.position, groundDistanceFoot, ground))
            {
    ...
    */
    ...
    private void ModifyTerrain()
    {
        ...
        if (leftFootDetector.isGrounded && !isSnow)
        {
            //Debug.Log("Left grounded");
           // Debug.Log("Pieds : " + leftFootIKPosition + " " + rightFootIKPosition);
            // Modify contourmap with raycasting.
            SetContourMap(leftFootIKPosition);
        ...
        if (rightFootDetector.isGrounded && !isSnow)
        {
            //Debug.Log("Right grounded");
            //Debug.Log("Pieds : " + leftFootIKPosition + " " + rightFootIKPosition);
            // Modify contourmap with raycasting.
            SetContourMap(rightFootIKPosition);
        ...
        */
        ...
        for (int i = -20; i < 20; i++)
        {
            for (int j = -20; j < 20; j++)
            {
                ...
                Ray ray = new Ray();
                ray.origin = columnPos + Vector3.up * 2f;
                ray.direction = Vector3.down;
                RaycastHit outHit;
                
                //Debug.Log("bizarre : " + targetFoot + " " +  rightFootIKPosition + " " + soleColliderRight.Raycast(ray, out outHit, 5.0f) );
                //Debug.DrawRay(ray.origin, ray.direction * 5.0f, Color.blue);
                if ((targetFoot - rightFootIKPosition).sqrMagnitude < 0.25f && soleColliderRight.Raycast(ray, out outHit, 5.0f))
                {
                ...
                else if ((targetFoot - leftFootIKPosition).sqrMagnitude < 0.25f && soleColliderLeft.Raycast(ray, out outHit, 5.0f))
                {
                    if (showFootprintDebug)
                    {
    ...
    private void SetHeightMap(Vector3 targetFoot)
    {
        ...
        for (int i = -20; i < 20; i++)
        {
            for (int j = -20; j < 20; j++)
            {
                //Debug.Log("contourmap " + contourmap_buffer[indZ + j, indX + i]);
